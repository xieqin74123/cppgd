...
void save_as_xyz (string path, const Gdata &data) {
    ...
    for (unsigned long molecule=0; molecule<data.get_data_num(); molecule++) {
        string name_temp;
        if (data[molecule].get_name()!="") {
            name_temp = data[molecule].get_name() + ".xyz";
        }
        else {
            name_temp = molecule + ".xyz";
        }
        ...
        for (unsigned long atom=0; atom<data[molecule].get_atom_num(); atom++) {
            output_temp << dictionary.element_dict(data[molecule][atom].get_atom_info());
            for (int i=0; i<3; i++) {
...
// 保存数据为npy格式
void save_as_npy (string path,
                  Gdata &data,
                  bool save_config=true) {

    // 检查统一
    if (!data.check_unified()) {
        data.unify();
    }
    ...
    shape[0] = data.get_data_num();
    shape[1] = data.get_max_atom();
    npy::SaveArrayAsNumpy(name_temp, false, shape.size(), shape.data(), vec_flat<short int>(data_temp));
    }
    ...
    if(data.structure_exist()) {
        ...
        shape[1] = data.get_max_atom();
        shape[2] = 4;
        npy::SaveArrayAsNumpy(name_temp, false, shape.size(), shape.data(), vec_flat<float>(data_temp));
        }
    }
    ...
    if(save_config) {
        ...
        conffile << data.get_data_num() << endl << data.get_max_atom() << endl << data.get_charge_type() << endl;
...
const Gdata read_chargemol_file (const string path) {
    ...
    while (!in_file.eof()) {
        ...
        if (line.find("Multipole analysis for each of the expansion sites.")!=string::npos) {
    ...
    molecule = new Molecule(get_real_name(path));
    if (molecule == NULL) {
...
const Gdata read_mol2_file (const string path) {
    ...
    while(!in_file.eof()) {
        ...
        if(line.find("@<TRIPOS>MOLECULE")!=string::npos) {
            ...
            //找到分子名称，并分配分子内存
            getline(in_file, line);
            molecule = new Molecule(get_real_name(line));
            if(molecule == NULL) {
                throw runtime_error("fail to allocate memory for new molecule!\n");
            }
            ...
            for(int i=0; i<atom_num; i++) {
                ...
                if(dot_pos == string::npos) {
                    ...
                    atom_type = dictionary.element_dict(get_in_range(word, 0, dot_pos));
                }
            ...
            if(skip_molecule){
                skip_molecule = false;
                cout << "warning: error found in molecule " << molecule->get_name() << ". skipped! ";
...
const Gdata read_gaussian_log_file (const string path, const string charge_type) {
    ...
    while(!in_file.eof()) {
        getline(in_file, line);
        if (line.find("Mulliken charges:") != string::npos || line.find("Mulliken charges and spin densities:") != string::npos) {
            // 跳过一行
            getline(in_file, line);
            mulliken_start = in_file.tellg();
        }
        else if (line.find("Hirshfeld charges, spin densities, dipoles, and CM5 charges") != string::npos) {
            // 跳过一行
            getline(in_file, line);
    ...
    Molecule molecule(get_real_name(path));
    Atom *atom;
    ...
    if (charge_type != "") {
        if (charge_type == "Mulliken") {
            ...
            cout << "Unsupported charge type, charge will be assigned as 0" << endl;
        }
    }
...
const Gdata read_gaussian_log_file (const vector<string> path_list, const string charge_type) {
    Gdata gd_output;
    Gdata gd_temp;
...
const Gdata merge (const Gdata &gd1, const Gdata &gd2) {
    ...
    for (unsigned long i=0; i<gd2_name.size(); i++) {
        if (find(match_table.begin(), match_table.end(), i) == match_table.end()) {
    ...
    for (unsigned long i1=0; i1<match_table.size(); i1++) {
        ...
        if (i2 == -1) {
            ...
            cout << "merging class ... " << setprecision(2) << fixed << i1/table_size*100 << "%\r";
            continue;
        }
        ...
        for (unsigned long j=0; j < gd1[i1].get_atom_num(); j++) {
            ...
            if (gd1.structure_exist()^gd2.structure_exist()) {
                if (gd1.structure_exist()) atom.add_position(gd1[i1][j].get_position());
                else atom.add_position(gd2[i2][j].get_position());
            }
            else if (gd1.structure_exist()) {
                if (gd1[i1][j].get_position() == gd2[i2][j].get_position()) atom.add_position(gd1[i1][j].get_position());
                else {
                    cerr << "error found in molecule " << gd1[i1].get_name() << " atom " << j << endl;
            ...
            if (gd1.charge_exist()^gd2.charge_exist()) {
                if (gd1.charge_exist()) atom.add_charge(gd1[i1][j].get_charge(), gd1[i1][j].get_charge_type());
                else atom.add_charge(gd2[i2][j].get_charge(), gd2[i2][j].get_charge_type());
            }
            else if (gd1.charge_exist()) {
                if (gd1[i1][j].get_charge() == gd2[i2][j].get_charge()) {
                    if (gd1[i1][j].get_charge_type() == gd2[i2][j].get_charge_type()) atom.add_charge(gd1[i1][j].get_charge(), gd1[i1][j].get_charge_type());
                    else {
                        cerr << "error found in molecule " << gd1[i1].get_name() << " atom " << j << endl;
        ...
        if (gd1.topo_exist()^gd2.topo_exist()) {
            if (gd1.topo_exist()) molecule.replace_topo(gd1[i1].get_topology_class());
            else molecule.replace_topo(gd2[i2].get_topology_class());
        }
        else if (gd1.topo_exist()) {
            if (gd1[i1].get_topology() == gd2[i2].get_topology()) molecule.replace_topo(gd1[i1].get_topology_class());
            else {
                cerr << "error found in molecule " << gd1[i1].get_name() << endl;
                throw runtime_error("topology not match!");
            }
        }

        // 合并dipole
        if (gd1.dipole_exist()^gd2.dipole_exist()) {
            if (gd1.dipole_exist()) molecule.add_dipole(gd1[i1].get_dipole());
            else molecule.add_dipole(gd2[i2].get_dipole());
        }
        else if (gd1.dipole_exist()) {
            if (gd1[i1].get_dipole() == gd2[i2].get_dipole()) molecule.add_dipole(gd1[i1].get_dipole());
            else {
                cerr << "error found in molecule " << gd1[i1].get_name() << endl;
